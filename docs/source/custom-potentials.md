...
`min_separation` is the separation below which the formula is not to be trusted. It is a class attribute and defaults to `0.0`, meaning the formula is trusted at every separation. A configuration gives a pair closer than it infinite energy, so placement and Monte Carlo never accept such a pair, and raises `ValueError` if asked for its forces. `Buckingham` sets it to the top of its short-range barrier in its constructor.
