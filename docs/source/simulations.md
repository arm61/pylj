...
Two analyses are computed on demand, from one frame or averaged over a trajectory. `rdf(bins=100, r_max=None)` returns the bin centres in metres and g(r), which is one where the atoms are spread as evenly as an ideal gas; `r_max` defaults to half the box. `structure_factor(q_max=None)` returns the wavevector magnitudes in inverse metres and S(q) at each. S(q) is evaluated at the wavevectors `2 pi (h, k) / L` commensurate with the box, for integer `h` and `k` not both zero; for each the amplitude is the sum of `exp(i q . r)` over the atoms, and S(q) is the square of its modulus divided by the number of atoms. Wavevectors of equal magnitude are averaged together, so the result holds one value per magnitude; the smallest magnitude has four wavevectors. `q_max` defaults to six times `2 pi sqrt(N) / L`, the wavevector that matches the mean spacing between the atoms.
