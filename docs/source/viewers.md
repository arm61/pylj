...
Panes that plot a quantity against time read it from the sample arrays on the simulation, which `sample()` fills. Each call records the current step in `samples.step`, so a loop may sample as often or as rarely as it likes. A molecular dynamics pane plots against `samples.step` times the timestep; a Monte Carlo pane plots against the step, since a Monte Carlo simulation has no timestep. The panes label their axes in the units the chapters use: Angstrom for lengths and picoseconds for time, with the mean squared displacement in Angstrom squared; the speed histogram and the scattering pane keep SI units.
