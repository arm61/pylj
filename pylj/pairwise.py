...
def minimum_image(separations: NDArray[np.float64], box: float) -> NDArray[np.float64]:
    ...
    """
    return separations - box * np.round(separations / box)


def dist(
    positions: NDArray[np.float64], box: float
    ...
    """
    i, j = np.triu_indices(positions.shape[0], 1)
    separations = minimum_image(positions[i] - positions[j], box)
    return np.linalg.norm(separations, axis=1), separations
