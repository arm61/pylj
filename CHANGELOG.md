...
- `md.calculate_temperature` divides the kinetic energy by (N - 1) k_B, the count of thermal degrees of freedom once the centre-of-mass motion is removed; the energy pane's kinetic term follows. Initial velocities are drawn from a normal distribution. `md.initialise` requires at least two particles.
