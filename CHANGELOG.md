...
- `md.at_rest`, which returns a configuration with its centre of mass at rest, and `MDSimulation` applying it to any configuration it is built from, so a simulation built from one a caller has modified starts in the zero-momentum frame as `initialise` does.
