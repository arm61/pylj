...
- Pair distances and forces are computed with vectorised NumPy. `pairwise.dist(position, box)` takes `(N, 2)` positions and returns the distances and the `(M, 2)` separations; `pairwise.calculate_pressure(virial, box, kinetic_energy)` is the instantaneous virial pressure, `(2 K + sum(f r)) / (2 L^2)`, whose kinetic term is `N k_B T / L^2` on average; it previously used the sampled temperature, which is defined over `N - 1` degrees of freedom, with `N` in the ideal term.
