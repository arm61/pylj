...
- `md.at_rest`, which returns a copy of a configuration with its centre of mass at rest. `MDSimulation` applies it to whatever it is built from, so a simulation built from a configuration a caller has modified starts with the centre of mass at rest, as one built by `initialise` does. `simulation.configuration` is therefore a copy rather than the object passed in, and a configuration whose atoms all share one velocity is refused, since removing the drift leaves it at rest.
